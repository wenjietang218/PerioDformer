...
class PerioDformer_backbone(nn.Module):
    ...
    def linear_head(self, z1, z2):
        ...
        for i in range(self.num):
            ...
            #         self.z = z2i
            #     else:
            #         self.z = torch.cat((z2i, self.z), dim=-1)  # self.z:[bs x nvars x d_model2 x 168]
            # self.z = self.z.permute(0, 1, 3, 2)  # self.z:[bs x nvars x 168 x d_model2]
            z2i=self.copy_z2(z2i)
            z2i = z2i.permute(0, 1, 3, 2)
            zi = torch.cat((z1, z2i), dim=-1)  # zi:[bs x nvars x 168 x (d_model+d_model2)]
            t = self.linears[i](zi)  # t : [bs x nvars x 168 x 1]
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # 预测头
        z = self.linear_head(z1, z2)  # z : [bs x nvars x C*num]
