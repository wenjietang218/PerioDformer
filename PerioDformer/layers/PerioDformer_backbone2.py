...
class PerioDformer_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int,
                 ...
                 subtract_last=False,
                 w=1, C=24, mlp_num=1, d_model2=16, d_ff2=128, dimension=16,
                 verbose: bool = False, **kwargs):

        super().__init__()

        self.w = w  # 用于输入复制的次数
        self.C = C  # 一周是多长 context_window要能被week整除
        self.period_num = int(context_window / self.C)  # 输入有几周
        ...
        self.intra_token_num = C  # 默认是24 一天为一组     week要能被patch_num整除

        self.inter_token_len = C
        self.inter_token_num = self.period_num

        self.num = int((target_window + self.C - 1) / self.C)  # 输出有几周
        ...
        self.linear = nn.Linear(d_model2, dimension)

        if self.mlp_num == 1:
            self.linears = nn.ModuleList(
                [LinearHeads1(int(d_model + self.period_num * dimension), 1) for i in range(self.num)])
        elif self.mlp_num == 2:
            self.linears = nn.ModuleList(
                [LinearHeads2(int(d_model + self.period_num * dimension), 1) for i in range(self.num)])
        elif self.mlp_num == 3:
            self.linears = nn.ModuleList(
                [LinearHeads3(int(d_model + self.period_num * dimension), 1) for i in range(self.num)])
        elif self.mlp_num == 4:
            self.linears = nn.ModuleList(
                [LinearHeads4(int(d_model + self.period_num * dimension), 1) for i in range(self.num)])
        elif self.mlp_num == 5:
            self.linears = nn.ModuleList(
                [LinearHeads5(int(d_model + self.period_num * dimension), 1) for i in range(self.num)])

    def cycle_division(self, z):  # z: [bs x nvars x seq_len]
        ar1 = []  # 存储输入倍增到w1倍的张量
        for i in range(self.period_num):  # 将输入序列按照周期进行分组 比如周就是以168为单位进行划分
            c = z[:, :, int(i * self.C): int((i + 1) * self.C)]
            c = torch.reshape(c, (c.shape[0], c.shape[1], c.shape[2], 1))
            ar1.append(c)  # ar1[i]: [bs x nvars x C x 1]

        z1 = torch.cat(ar1, dim=-1)  # z:[bs x nvars x C x period_num]
        ...
        z2 = torch.cat(ar1, dim=-1)  # z:[bs x nvars x C x w*period_num]
        return z2, z1
    ...
    def linear_head(self, z):
        zr = []
        for i in range(self.num):  # 预测的每一周需要一个预测头
            t = self.linears[i](z)  # t : [bs x nvars x C x 1]
    ...
    def encoder2(self, z2):
        ...
        z2 = z2.permute(0, 1, 3, 2)  # z2:[bs x nvars x period_num x d_model2]
        z2 = self.linear(z2)  # z2:[bs x nvars x period_num x dimension]
        z2 = torch.reshape(z2, (
            z2.shape[0], z2.shape[1], z2.shape[2] * z2.shape[3], 1))  # z2:[bs x nvars x period_num*dimension x 1]

        z2 = self.copy_z2(z2)  # z:[bs x nvars x period_num*dimension x C]
        z2 = z2.permute(0, 1, 3, 2)  # z2:[bs x nvars x C x period_num*dimension]

        return z2

    def norm(self, z):
        # norm
        if self.revin:  # 只有多变量才需要 单变量可以将revin设置为false
            z = z.permute(0, 2, 1)  # 当考虑多变量的时候 输入的z是三维的
            z = self.revin_layer(z, 'norm')  # 对每一个时间点的 多维数据做nrom
            z = z.permute(0, 2, 1)
        return z
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # cycle_divison
        z1, z2 = self.cycle_division(z)  # z:[bs x nvars x C x period_num]  z1是周期内 z2是周期间)

        # encoder 1
        z1 = self.encoder1(z1)  # z1:[bs x nvars x C x d_model]

        # encoder 2
        z2 = self.encoder2(z2)  # z2:[bs x nvars x C x period_num*cross]

        z = torch.cat((z1, z2), dim=-1)  # z:[bs x nvars x C x (d_model+period_num*cross)]

        # 预测头
        z = self.linear_head(z)  # z : [bs x nvars x C*num]
