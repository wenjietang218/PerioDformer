...
class PerioDformer_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int,
        ...
        # the linear layer after inter_encoder
        ...
        print("线性头的参数量为：", count_params(self.linears))
    ...
    def periodic_division(self, z):  # z:[bs x nvars x seq_len]
        if self.add_flag == 1:  # use the mean of the previous periodic sequences to supplement the missing time steps at the end
            ...
            for i in range(add_num):
                ...
                sum = copy.deepcopy(z[:, :, 0])
                for j in range(self.period_num - 1):
                    t = copy.deepcopy(z[:, :, int(seq*j)])
    ...
    def copy_z2(self, z): # copy z of C times
        zr = []
    ...
    def linear_head(self, z): # accurate predictors
        zr = []
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # inter_encoder
        z2 = self.encoder2(z2)  # z2:[bs x nvars x C x period_num*dimension]

        z = torch.cat((z1, z2), dim=-1)  # z:[bs x nvars x C x (d_model+period_num*dimension)]

        # accurate predictors
        z = self.linear_head(z)  # z : [bs x nvars x target_window]
